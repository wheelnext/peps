...
variant has no properties and is therefore always compatible.

In addition to these variant wheels, a ``torch-2.13.0-variants.json``
file is published with the merged variant metadata files. It has the
same contents as the example, except that the ``variants`` dictionary
includes all variant labels and their properties.

When requested to install ``torch``, the installer does, in order:
...
3. The ``torch-2.13.0-variants.json`` is found in the response and it is
   downloaded. Its contents are read to determine the meaning of variant
   labels, as well as the namespace preference order.

4. The lists of compatible features and feature values is obtained for
   ...
   - The ``cuda*`` wheels contain a
     ...
     whether to determine whether a compatible runtime and one of the
     compatible GPUs are available.  They also contain a
     ``priority :: order`` feature that is always compatible. If either
     the driver, a compatible runtime or a compatible GPU are not
     available, the wheels are removed from the list.

   - Similarly, the ``rocm*`` wheels contain a ``amd :: rocm_version``
     ...
     before. The If either the driver, a compatible runtime or a
     compatible GPU are not available, the wheels are removed from the
     list.

   - The ``null`` wheel has no properties, and it is therefore always
     compatible.

   On a system with a compatible NVIDIA GPU and a CUDA runtime, at least
   one of the ``cuda*`` wheels and the ``null`` variant will remain on
   the list.
...
5. If variant wheels with multiple different labels remain on the list,
   ...
   properties. In this case, the ``nvidia`` properties do not provide
   a total ordering, so the ``priority :: order`` property is used to
   order these wheels. The ``null`` wheel always sorts last, so it is
   selected only if no compatible GPU is available.
...
6. If at this point multiple wheels with the same label remain on the
   ...
   here. It could happen, for example, if the same wheel variant was
   provided both in ``abi3`` and ``cp315``, when installing for Python
   3.15, the installer would choose between these two based on the
   Python tag.
