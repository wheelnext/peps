...
1. The remote index is queried to determine available wheels. It is
...
4. The lists of compatible features and feature values is obtained for
   ...
   For example, if the user has a compatible NVidia GPU, then the
   appropriate ``cuda*`` and ``null`` wheels remain on the list, while
   ``rocm*`` wheels are removed.
...
5. If variant wheels with multiple different labels remain on the list,
   ...
   properties, and the compatible wheel corresponding to the highest
   CUDA version is selected. The ``null`` wheel always sorts last, so
   it is selected only if no compatible GPU is available.
...
7. The metadata for selected wheel is processed. The variant properties
   corresponding to the wheel (as stored in the ``variants`` dictionary)
   are used to process `environment markers`_. This results in
   additional CUDA-related dependencies being selected.
