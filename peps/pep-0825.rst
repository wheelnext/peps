...
2. ``variant_properties``: a set of all
   ...
   corresponding to the variant label that are supported by the target
...
The ``variant_label`` marker MUST be matched using the ``==`` and
``!=`` operators only. In particular, ``in`` and ``not in`` MUST NOT be
used with it, as they would perform substring matching.

The ``variant_properties``, ``variant_features`` and
``variant_namespaces`` markers MUST be matched using the ``in`` and
``not in`` operators only.
