...
On the discussion thread concerning this PEP, `Paul Moore has made an
extensive argument regarding the data model
<https://discuss.python.org/t/pep-825-wheel-variants-package-format-split-from-pep-817/106196/111>`__.
Essentially, Paul is saying that this data becomes a wheel-level
...
data is project-level metadata which, which is copied into the wheel.
Similarly to other project metadata, there is no reference to other
wheels. And even if, for example, variant wheels using some of the
specified namespaces are missing from a release completely, that is
fine, nothing depends on specific other wheels existing.
...
1. It is possible to generate the index-level metadata file based on
   available wheels alone, wthout any additional input data. Notably,
