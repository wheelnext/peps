...
extension sets). To avoid extending wheel filename length, the property
lists are stored inside the wheel and mapped to a short label that is
chosen by the package maintainer and intended to be human-readable.
...
properties as data without having to change the algorithm. Ordering can
be both controlled on feature level (for example, indicating that a
wheel built for higher CPU baseline is preferred over a wheel built for
instruction sets being part of that baseline) and at value level (for
example, indicating that a wheel built for x86-64-v4 CPU is preferred
over one built for x86-64-v2, provided that both are supported).

While feature and value ordering will be provided, namespaces are
governed independently and considered on equal footing, and therefore
there will be no standard ordering for them. Instead, the package
maintainer will decide which namespaces have higher priority, and
therefore which variants will be preferred. For completeness, the
specification also permits the maintainers to override the ordering for
