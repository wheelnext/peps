...
6. Obtain the provider information corresponding to these namespaces
   ...
   ``{"nvidia", "x86_64"}``.
7. For each of the providers:

   - If the provider is vendored in the installer, the subsequent
     queries will be directed to the vendored provider.
   - If it is not vendored but the user opted in to a given provider,
     install the provider package in an isolated environment. The
     subsequent queries will be directed to the provider plugin in this
     environment.
   - Otherwise, the provider package is not used and the properties are
     considered to be incompatible.

8. Query the provider to determine whether the properties of the variant
   wheels on the list are compatible. Discard the variant wheels that
   are incompatible. Let's say that both ``*-cu130.whl`` and
   ``*-x86_64_v4.whl`` are compatible.
9. If any variant wheels remained, order them primarily based on
...
10. If there are multiple wheels with the selected label but different
    Platform Compatibility Tags, select the most appropriate of them
    based on Platform Compatibility Tags.


Using static compatibility info
-------------------------------
...
version. This may require installers to navigate through a complex maze
of interdependencies to find package versions that can all be installed
simultaneously.
...
4. The design provides both for dynamic providers that need to be able
   to determine system compatibility at runtime, and for static
   providers that embed all the necessary information in the wheel
   itself. It also permits defining special cases, such as the
   ``abi_dependency`` provider that solve specific problems without
   adding significant complexity to the specification.
5. The reference method of implementing dynamic providers is through
   installable Python packages, which provides the flexibility needed
   for package maintainers to extend variant wheel support as necessary,
   as well to facilitate convenient testing. However, we do realize that
   we cannot allow arbitrary code execution at install time, so these
   plugins are opt-in.
...
plugins. A governance model to maintain a repository of trustwhile
...
variants specificaiton, as well as a command-line tool to convert
