...
This would combine the standardized configuration specified above with
build-backend specific configuration that controls the actual build
configuration. Once the build is finished, the build backend would copy
the metadata into the wheel, creating a ``*.dist-info/variant.json``
