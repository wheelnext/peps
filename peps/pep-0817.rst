...
This PEP presents the minimal scope required to meet modern
heterogeneous system needs. It leaves aspects beyond the minimal scope
to evolve via tools or future PEPs. A non-exhaustive list of these
aspects include:
...
a given system. The question that shapes the rest of the design is where
that knowledge lives, and who is able to add to it.

One point is worth making at the outset, because it is easily lost:
every approach described here requires a PEP. They differ in what
requires one *next time*. An approach that standardizes the individual
compatibility axes requires a further round of standardization, followed
by an implementation in every installer, each time hardware or a library
introduces an axis that is not already covered. An approach that
standardizes an interface requires that round once.
...
.. list-table::
    ...
    * - Fixed set of axes
      - A PEP, then an implementation in every installer
      - Installer maintainers
      - No
    * - Logic shipped per package
...
A PEP could define an extended set of compatibility markers - CUDA
runtime version, GPU architecture, CPU instruction set level, and so
on - together with the semantics for matching them. Wheels would declare
their requirements against these markers, and each installer would
implement the detection needed to evaluate them. The specification would
not need to say how a tool measures any particular marker; that would
remain an implementation matter for each tool.
...
The cost is that the specification becomes the gatekeeper for every
compatibility axis. Each new axis requires agreement across the
packaging community followed by an implementation in each installer, and
a newly specified axis is only as useful as its slowest widely deployed
implementation. The axes in demand today already extend well past GPUs:
CPU instruction set levels, BLAS and LAPACK implementations, OpenMP
runtimes, C++ ABIs, and matching against the ABI of a dependency such as
PyTorch_. Hardware vendors would depend on the standards process in
order to ship optimized wheels, on a timescale measured against their
own release cadence.
...
variant selection via source distribution`_ already works this way.
...
ensuring that NumPy_ and SciPy_ agree on a BLAS implementation, or that
...
`Package names as variants`_ describes what projects do today. A more
...
This has the attraction of working within the format as it stands. It
requires no filename changes and no metadata changes, and publishers
retain control over what they publish. The drawbacks described in
`Package names as variants`_ still apply: name squatting becomes easier,
...
Two further problems are specific to the mapping. A project depending on
any variant of another project still has no way to express that, and
must either choose one name or enumerate them all. More fundamentally,
the mapping needs a source of truth: something must still determine
which name is appropriate for the current system. That is the question
this section is about, so this approach relocates it rather than
answering it.
...
than values a tool detects. Wheels would carry markers as above, and the
installer would compare them against a description of the environment
supplied by hand or produced by a separate tool. No detection code would
run anywhere during installation.

This has the smallest trust surface of any approach considered, since no
code runs in order to make a compatibility decision, and it describes
...
mechanisms such as those described in `Spack / Archspec`_, and prefer an
explicit, auditable description to anything discovered at install time.
...
and instruction set level. That is precisely the knowledge this proposal
exists to stop requiring, and ``{tool} install <package>`` selecting a
working wheel without it is the goal stated in the abstract.

The design does not treat this as an alternative to be discarded. A
static description of the environment is a first-class part of it, as
described in `Using static compatibility information`_, and it is needed
regardless of any position on trust: installing for a target that is not
the machine running the installer - building container images for other
hardware, populating a wheelhouse, or reproducing an environment in
CI - requires exactly this mechanism.


The approach taken: variant providers
-------------------------------------

The design described in the next section standardizes an interface
rather than a list of axes. Compatibility within a namespace is
determined by a provider, and a new compatibility axis is introduced by
publishing or updating a provider rather than by amending a
specification.
...
for a new accelerator, and correct compatibility rules when they change,
...
governance. System inspection moves into dependency resolution, where it
was previously confined to the tags a tool computes for itself. Users
may need to obtain a provider in order to install a package they
previously installed without one. And because the properties driving
selection are held in metadata rather than in the filename, determining
which wheel a given system will select requires reading that metadata
rather than a directory listing; :pep:`825` discusses the alternatives
considered for this.
...
so, and that question remains genuinely open. It is raised here because
it is the most contested aspect of the design; the mechanisms themselves
belong to the subsequent `Standards Track PEPs for wheel variants`_.
...
resolution is not new: determining whether a ``manylinux`` or
``musllinux`` wheel is compatible already requires probing the running
system, down to reading the interpreter's ELF headers to identify the C
library and its version, and installers perform that inspection today
from vendored code. A provider an installer vendors is much the same
kind of artifact. What is new is that a provider may instead come from a
third party, named by the package being installed.
...
governed and that is worth capturing. A static description of the
environment removes the question entirely for users who want that, at
the cost of requiring them to produce one.

This PEP does not attempt to settle that trade-off. What would change
the reasoning above is a demonstration that the compatibility axes in
demand can be enumerated and kept current through the standards process
at the rate hardware actually changes, or evidence that a static
description of the environment is sufficient for the general case rather
than for those users who already prefer it.
...
of the standard, nor does it provide normative guidance on
...
.. figure:: pep-0817/conceptual_diagram_installers.png
    ...
    :alt: A diagram showing installing a package including variant wheel
          handling. It is split into three columns: Developer, Installer
          and Install-time providers. The diagram starts with Developer
...
4. Find the ``{name}-{version}-variants.json`` file corresponding to
