...
every approach described here requires standardization. They differ in
how much, and in what requires more of it *next time*. Standardizing the
compatibility axes themselves means a document for each, since the rules
governing NVIDIA CUDA compatibility have little in common with those
governing AMD ROCm and less still with x86-64 instruction set levels,
and a further document each time hardware or a library introduces an
axis not already covered - followed in every case by an implementation
in each installer. Standardizing an interface means one document, once.
...
have little in common with those governing AMD ROCm, and less still with
x86-64 instruction set levels or Arm feature flags. Each needs different
expertise to write and different reviewers to assess it competently, and
a single document covering them all would be difficult to review as a
whole. A realistic first round is therefore several PEPs: plausibly one
per accelerator vendor, and at least one more for CPU instruction sets,
which may not remain single as architectures diverge.
...
of a dependency such as PyTorch_. Hardware vendors would depend on the
standards process in order to ship optimized wheels, on a timescale
measured against their own release cadence.
...
variant selection via source distribution`_ already works this way, and
`wheel-stub <https://github.com/wheelnext/wheel-stub>`__ packages it as
...
something must establish that the resolved name is ``torch_cu128``
rather than ``torch-cu12`` or ``torch_cuda_128``, and that whatever
convention is chosen holds for every project using the mechanism. How
that would be agreed between projects and kept consistent across
installers is unclear, and the suffix carries meaning that nothing in
the metadata records. More fundamentally, the mapping needs a source of
truth: something must still determine which name is appropriate for the
...
does not, so today a project that merely depends on PyTorch_ can undo
...
The Wheel Variants design does not treat this as an alternative to be
discarded. A static description of the environment is a first-class part
of it, as described in `Using static compatibility information`_, and it
is needed regardless of any position on trust: installing for a target
that is not the machine running the installer - building container
images for other hardware, populating a wheelhouse, or reproducing an
environment in CI - requires exactly this mechanism.
...
The Wheel Variants design, described in the next section, takes two of
the approaches above rather than one. Variant providers are the default
mechanism, and a user-declared description of the environment is
supported alongside them as a first-class path rather than as a
fallback.

Providers are the default because they are the only approach that leaves
``{tool} install <package>`` working for a user who does not know which
accelerator or instruction set their machine has, without making the
specification the gatekeeper for every compatibility axis. Each of the
other approaches gives up one half of that: a fixed set of axes keeps
the declarative model but puts every future axis through the standards
process, per-project logic multiplies the same work by the number of
projects, and separate names relocate the question rather than answering
it.
...
need a route that involves none. A user whose hardware is misdetected
needs a way to say so.

The two fit together rather than merely coexisting, because they supply
the same thing. A provider's output is a set of supported properties in
preference order; a static description declares that set directly.
Selection consumes one form or the other without needing to know which,
so the declared path substitutes at a single point instead of running as
a second mechanism in parallel. That is what makes it reasonable to
offer both without doubling what has to be specified, implemented and
reasoned about.
