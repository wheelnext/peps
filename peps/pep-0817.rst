...
within a single package version. Variant labels SHOULD be specified at
wheel build time, as human-readable strings. The label ``null`` is
reserved for the null variant and MUST use an empty set of variant
properties.
...
alter who takes responsibility of vetting the provider code, and hence
the maintenance effort or number of packages/maintainers that need to be
