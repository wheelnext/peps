...
:doc:`packaging:specifications/binary-distribution-format` that lets a
project publish several wheels for the same version, distinguished by
properties the platform tags cannot carry, and lets installers choose
between them automatically. It comprises:

- **Variant wheels**, which declare the variant properties they were
  built for alongside the existing platform tags.
...
- A **static description of an environment**, which a user may supply in
  place of any inspection. It is needed when installing for a machine
  other than the one running the installer, and available to anyone who
  would rather no provider code ran at all.

The goal is for the obvious installation command (``{tool} install
<package>``) to select the most appropriate wheel, whether that choice
...
as the umbrella for a sequence of four Standards Track PEPs, supplying
the motivation, the prior art, and the reasoning behind the design
choices those PEPs make, so that each of them can be read against a
shared context rather than restating it. It aims to give a concise
high-level overview of wheel variants; it does not cover the complete
functionality proposed as part of the standard, nor does it provide
normative guidance on implementing it.

The four PEPs are:

- :pep:`PEP 825: Package Format <825>`: changes to the wheel format, the
...
- Providers: the governance of variant namespaces; extending variant
  metadata with provider information; introducing providers that supply
  compatible properties statically via variant metadata or dynamically
  via opt-in Python packages, along with the security considerations for
  their use; defining the relevant plugin API; introducing a special
  provider concerned with ABI compatibility.
...
Of these, :pep:`825` has been accepted as Provisional; the remaining
...
The first of these concerns the encoding only. The sequence requires
installers to accept a static description of an environment's variant
properties; what it leaves open is the form that description takes.
...
is why both already exist as workarounds today; their cost falls
elsewhere.
...
since the rules governing NVIDIA CUDA compatibility have little in
common with those governing AMD ROCm and less still with x86-64
instruction set levels, and a further PEP each time hardware or a
library introduces an axis not already covered - followed in every case
by an implementation in each installer. Standardizing an interface means
one PEP, once.

.. list-table::
    :header-rows: 1
    :widths: 18 12 26 22 22
    ...
    * - Variant providers
      ...
      - Yes, unless the provider is vendored or supplied statically

The table states what each approach costs and where the work lands. The
sections below take each in turn; `The approach taken`_ explains the
...
The design set out further down in `The wheel variants design`_ takes
...
Why variants stay out of the resolver's search
'''''''''''''''''''''''''''''''''''''''''''''''
...
the ``abi_dependency`` provider described in `Package ABI matching`_,
...
belong to the PEPs listed in `Scope and Standards Track PEPs`_.
...
out of the resolver's search`_. The third is that installers assume
...
one the design does address; see `Package ABI matching`_ below.
...
search`_), because doing so may be difficult to implement. Support for
``abi_dependency`` is therefore optional for installers: one that does
not implement it selects a variant in the ordinary way and reports any
...
listed in `Scope and Standards Track PEPs`_. The most significant are
summarized here.
...
A second concern is name squatting, which `Package names as variants`_
...
decision, open to the same scrutiny as any other. Which parties may
define a namespace, and how reserved names such as ``abi_dependency``
are administered, is covered by the Providers PEP.
...
wheels at the time of writing reject wheels carrying it. This makes it
possible to publish variant wheels on an index alongside non-variant
wheels without risk of them being installed accidentally.
