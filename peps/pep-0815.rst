...
1. **Python tag:** `The Python tag indicates the implementation and
   ...
   (e.g., ``cp313``)
2. **ABI tag:** `The ABI tag indicates which Python ABI is required by
   any included extension
   modules <https://packaging.python.org/en/latest/specifications/platform-compatibility-tags/#abi-tag>`__
   (e.g., ``cp313``, ``none``)
3. **Platform tag**: `Operating system and architecture - In its simplest
   form:
...
:pep:`694` proposes adding such a mechanism for multiple wheels within a
single package, but extending it to multiple packages is not a goal.
