...
various projects operating purely on wheels, as well as users using
the ``--only-binary`` option have been relying on the assumption that
no code external to the system will be executed while resolving
dependencies, installing a wheel or otherwise processing it. To uphold
this assumption, the proposal explicitly requires that provider plugin
packages are never installed without explicit user consent.
...
Notably, the specification suggests vendoring or locking popular
provider plugin versions at the installer level. This permits the wheels
using them to be processed either without executing code external
to the tools at all, or the code to be audited on the same level
as the tools themselves. Furthermore, the specification permits using
static configuration as input to skip running plugins altogether.
