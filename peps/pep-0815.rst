...
2. Variants that can always be installed, and therefore only provide
   a choice between different installation options. This could
   include software built against different BLAS / LAPACK providers
   or debug builds of packages.
...
risks highlighted in `security implications`_ section. The proposed
mitigations incur a cost on installer implementations, making it
desirable to avoid plugin use in the second class where it is not
strictly necessary.
