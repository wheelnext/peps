...
- ``requires: list[str]``: A list of one or more package
  ...
  dependency that remains after filtering MUST always evaluate to the
  same default ``plugin-api`` value.
...
- ``plugin-api: str``: The API endpoint for the plugin. If it is
  ...
  determined by normalizing the package name from the first dependency
  specifier in ``requires`` and then replacing all ``-`` characters with
  ``_`` in the normalized package name.
...
that include static lists of compatible features and their values as
part of their metadata, installable Python packages MUST be provided for
