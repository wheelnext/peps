...
This PEP is a followup to :pep:`825` that defines how variant properties
are governed and how their compatibility is determined. This is done
either via static feature and feature value compatibility lists
...
plugins. Additionally, an ABI Dependency Variant Provider is defined
to facilitate builds of the same package against different dependency
versions.
...
system. This PEP aims to fill this gap.
...
`provider information`_ dictionary in `variant metadata`_.

Each of these providers either defines a static list of compatible
features and their values, or specifies a Python plugin package that
is queried to obtain said list. The plugin packages are not used by
default. However, tools are permitted to vendor, reimplement or consider
a subset of plugin packages to be trusted, and therefore enable them by
default. Further recommendations on this matter will be provided by a
subsequent PEP. Users are provided with the ability to specify
additional packages as trusted, or to provide a static list of
compatible features and their values to use instead of querying the
plugin.
...
1. A variant provider MAY be enabled or disabled by default, as
   determined by the ``optional`` key in the `provider information`_
   dictionary. The tools MUST provide a way to explicitly enable a
   provider, and MAY provide a way to disable one. If a provider is
   disabled, the tools MUST assume that the list of compatible features
   is empty and they MUST NOT use the provider in any way. Otherwise,
   proceed to step 2.

2. A variant provider's metadata MAY include a static list of compatible
   features and their values as the ``static-properties`` key in the
   `provider information`_ dictionary. If that were the case, the tools
   MUST use the static lists exclusively. Otherwise, proceed to step 3.

3. The tools SHOULD implement a way for the user to provide a static
   list of compatible features and their values. If the user provided
   said list, the tools MUST use it exclusively. Otherwise, proceed to
   step 4.

4. If no static list of compatible features and their values is
   provided, the variant provider metadata MUST specify a Python package
   that provides a variant provider plugin in the ``requires`` key in
   the `provider information`_ dictionary. The tools MAY choose to
   vendor or reimplement a chosen subset of plugin packages. If that
   were the case, they SHOULD obtain the list from their implementation.
   Otherwise, proceed to step 5.

5. The tools MAY consider some of the required provider plugin packages
   (and their dependencies) to be trusted by default, and install them
   automatically. They SHOULD provide a way for the user to permit
   installing additional provider packages. They MUST NOT install or use
   any provider packages that are not considered trusted. The tools
   SHOULD use an isolated virtual environment while installing provider
   plugin packages.

6. The tools MUST query the trusted and installed provider packages
   via the `provider plugin API`_ to obtain the list of compatible
   features and feature values. For providers that were not trusted,
   the tools MUST assume that the list of compatible features is empty.
...
subsection. Therefore, the metadata has the following structure:
...
- ``static-properties: dict[str, list[str]]``: The dictionary of static
  ...
  specified, then the order for all features MUST be specified in
  the ``feature-order`` key.
...
- ``plugin-api: str``: The API endpoint for the plugin. If it is
  ...
  determined by
  :ref:`normalizing the package name <packaging:name-normalization>`
  from the first dependency specifier in ``requires`` and then replacing
  all ``-`` characters with ``_`` in the normalized package name.
...
published in the same indexes as the packages using them. In
...
behavior with the published provider.
...
b. as the value of an installed entry point, as described in
   `variant plugin discovery`_.


Variant plugin discovery
''''''''''''''''''''''''

The Python packages providing variant provider plugins SHOULD install
an entry point to facilitate discovery by variant-related utilities.
The entry point MUST be placed in the ``variant_plugins`` group, its
name is insignificant and its value MUST be the plugin's `API
endpoint`_. For example, a plugin can declare the following entry
point:

.. code:: toml

    [project.entry-points.variant_plugins]
    provider_variant_x86_64 = "provider_variant_x86_64"
...
to determine the API endpoints to call. They MAY use the associated
package distribution metadata to determine which package provides a
given plugin.
...
``default-priorities.namespace`` dictionary, and can only appear in the
``variants`` dictionary.
...
defaults to being inferred from the package name, addressing a criticism
...
Admittedly, such attacks can already be done to the package's
dependencies. However, in some cases the affected tools are executed
with elevated privileges (such as when installing packages for
multi-user systems), while the package itself will only be run with
regular user privileges. Therefore, variant provider plugins could
introduce a Remote Code Execution vulnerability with elevated
privileges.
...
build dependencies are being installed and executed. However, various
tools operating purely on wheels, as well as users using tool-specific
options to disable the use of source distributions, have been relying on
the assumption that no such code execution will happen. To uphold this
assumption, the proposal makes plugin packages opt-in.
...
improve user experience, the PEP permits tool maintainers to provide
opt-out experience for selected plugins. A subsequent PEP will give
further recommendations for improved user experience.
...
do not need to be aware of Platform compatibility tags.
...
This PEP is concerned with installing wheels only. A subsequent PEP will
address building variant wheels.
