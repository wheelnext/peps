...
features and their values, or specifies a requirement referring to a
Python plugin package. The name in the requirement uniquely identifies
the provider, and the version specifier declares the required feature
level. For security, the plugin packages MUST NOT be fetched from an
index by default. Tools are permitted to vendor, reimplement or consider
a set of trusted of plugin packages, and enable them by default. Users
can opt-in untrusted providers, or to provide a static list of
