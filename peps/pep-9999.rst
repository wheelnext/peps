...
data for binary packages, in the form of variant properties, but does not
define how to obtain the matching information for the system. This PEP defines 
how to determine which variant properties are compatible with a particular
system and how variant namespaces are governed.
...
3. Variants that express compatibility with different dependency
   ...
   distribution can narrows to specific versions of a dependency
   during build.
...
features and their values, or specifies a requirement referring to
a Python plugin package. The name in the requirement uniquely identifies the
provider, and the version specifier declares the required feature level. For
security, the plugin packages MUST NOT be fetched from an index by default. Tools
are permitted to vendor, reimplement or consider
a set of trusted of plugin packages, and enable them by
default. Users can opt-in untrusted providers, or to provide a static list of
compatible features and their values to use instead of querying the
plugin. Further recommendations on this matter will be provided by a
subsequent PEP.
...
1. A variant provider MAY be enabled or disabled by default, as
   ...
   dictionary. The tools SHOULD provide a way to explicitly enable a
   provider, and MAY provide a way to disable one. If a provider is
   disabled, the list of compatible features is empty. Otherwise,
   proceed to step 2.
...
2. A variant provider's metadata MAY include a static list of compatible
   ...
   `provider information`_ dictionary. In this case, tools
...
name being the recommended namespace name, and its value being the
plugin's `API endpoint`_. For example, a plugin can declare the
