...
5. The tool SHOULD provide a way for the user to permit installing
   ...
   packages that are not considered trusted. It SHOULD create one isolated
   virtual environment per unique (variant provider package name, version)
   pair. Different provider packages use separate environments; two versions
   of the same provider package also use two separate environments.
