...
2. A variant provider's metadata MAY include a static list of compatible
   ...
   the static lists exclusively. Otherwise, proceed to step 3.
...
4. If no static list of compatible features and their values is
   ...
   that provides a variant provider plugin. The tools MAY choose to
   vendor or reimplement plugin packages. If that is the case, they
   SHOULD obtain the list from their implementation.  Otherwise, proceed
   to step 5.
...
Variant metadata must include a ``providers`` key. It corresponds to a
dictionary, in which the keys are namespaces, the values are
dictionaries with provider information. It specifies how to install and
use variant providers.  All the namespaces found in
``default-priorities.namespace`` MUST be listed in this dictionary,
and other keys MUST NOT be present.
...
to the same values. When combining metadata, the resulting ``providers``
...
The ``foo-1.2.3-variants.json`` example from :pep:`825` can be extended
...
``default-priorities.namespace`` dictionary, and can only appear in a
built wheel variant property.
...
options. For example, build frontends may prefer reusing the isolated
