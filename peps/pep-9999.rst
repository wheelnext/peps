...
to Python packages or is one of the `reserved providers`_. In the case
of Python package providers (also called provider plugins), an API
...
3. The tool MUST read the ``builtin`` key from the `provider
   information`_ dictionary. If it is ``true``, the tool MUST use the
   internal provider implementation if supported, or assume that no
   features are compatible if not. If it is not ``true``, proceed to
   step 4.
...
- ``builtin: bool``: Whether the provider is a reserved provider
  specified in a PEP. Defaults to ``false``. If it is ``true``, then the
  provider is identified by the namespace, and MUST behave as specified
  in the PEP defining it. If the tool does not recognize the particular
  provider, it MUST assume that its list of compatible features is
  empty.
...
- ``builtin``, with the value of ``true``
...
.. code:: json5

    {
      ...
      "default-priorities": {
        ...
        "namespace": ["x86_64", "aarch64", "blas_lapack", "abi_dependency"],
      },
...
Reserved providers are special providers that cannot be implemented
using the plugin API. They are identified by namespace used, provided
that the respective entry in the `provider information`_ indicates the
use of ``builtin`` provider. Packages can use the same namespace for
other providers, if no ``builtin`` key is found in the provider
information.

All reserved providers are OPTIONAL. Tools that choose to implement them
MUST follow the PEP implementing them. Tools that do not recognize a
particular reserved provider MUST treat the variants using it as
incompatible, and HOULD inform users when such wheels are skipped.

The following reserved variant providers are defined:
...
Subsequent PEPs may define additional reserved providers.
...
versions or version ranges of a dependency. The ordering between
features and feature values is undefined, and the ``abi_dependency``
namespace should be listed after all the ordering-significant namespaces
in the ``default-priorities.namespace`` list. Package maintainers SHOULD
NOT be publishing variant wheels where two different ``abi_dependency``
property sets could be compatible with the same system.
...
further `reserved providers`_ in the future, and to keep the provider
...
- 22-Sep-2026
  ...
  - The `ABI Dependency Variant Provider`_ now requires an explicit
    ...
    keys, and it was generalized into one of the possible `reserved
    providers`_.
