...
1. The tools SHOULD implement a way for the user to provide a static
   list of compatible features and their values for a particular
   provider. Standardizing this format is left to a future PEP. If the
   user provided said list, the tools MUST use it exclusively.
   Otherwise, proceed to step 2.

2. Read the ``optional`` key from the `provider information`_ to
   ...
   compatible features is empty. Otherwise, proceed to step 3.

3. Read the ``static-properties`` key from the `provider information`_
   dictionary. If it is present, the tools MUST use the static lists of
   properties therein exclusively. Otherwise, proceed to step 4.
