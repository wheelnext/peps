...
behavior with the published provider. The reimplemenetations and
vendored providers MUST be matched to the published providers using the
...
the entry point shown above, ``variantlib update-pyproject-toml --add x86_64``
would add the following metadata to ``pyproject.toml``:
