...
executed with elevated privileges, for example when installing packages
for multi-user systems. On the other hand, the code in installed
packages will be used with regular user privileges afterwards.
Therefore, variant provider plugins could introduce a Remote Code
Execution vulnerability with elevated privileges.

A similar issue already exists in the packaging ecosystem when packages
are installed from source distributions, where build backends and other
build dependencies are installed and executed. However, various tools
operating purely on wheels, as well as users using tool-specific options
to disable the use of source distributions, have been relying on the
assumption that no such code execution will happen. To uphold this
